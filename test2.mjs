		...
		const expandedStates = state.expand(state => {
			...
			for(let i = 0; i < fields.length; i++) {
				...
				for(let j = 0; j < moves.length; j++) {
					const move = moves[j];
					const newPos = car.direction === HORIZONTAL ? y * width + (x + move) : (y + move) * width + x;
					newStatesCars.push([[car.id], [((i << 16) | (newPos & 0xFFFF)) >>> 0]]);
				}
			}
...
const FIELD_EMPTY = 0;
const FIELD_WALL = 255;
const FIELD_PRIMARY_CAR = 1;
const HORIZONTAL = 0;
const VERTICAL = 1;
...
class State {
	...
	constructor_root(width, height, allCars, carPositions, walls, exitPosition) {
		...
		for (let i = 0; i < allCars.length; i++) {
			pack.push([allCars[i], carPositions[i]]);
		}
		pack.sort((a, b) => a[0].id - b[0].id);
	...
	#calculateHashCode() {
		...
		// }
		// @Porffor
		Porffor.hashFields(this.#width, this.#height, this.#fields, State.#__tempUint32Array);
		this.#hashCode = State.#__tempUint32Array[0];
	...
	getCarMoveOptions(direction, position, size) {
		...
		for (let step = 1; step <= maxSteps; step++) {
			...
			if (this.#fields[index] !== FIELD_EMPTY) break;
	...
	expand(expander) {
		...
		const result = [];
		for (let i = 0; i < newStatesCars.length; i++) {
			const [newCarIds, newCarPositions] = newStatesCars[i];
			const newState = State.new_isDifference(this, newCarIds, newCarPositions);
	...
	toString() {
		...
		// @Porffor
		const numChunks = Math.ceil(this.#fields.length / this.#width);
		const lines = [];
		for(let i = 0; i < numChunks; i++) {
			const chunk = [];
			for(let j = 0; j < this.#width; j++) {
				const index = i * this.#width + j;
				if(index >= this.#fields.length) break;
				const v = this.#fields[index];
				let display;
				if(v == FIELD_EMPTY) {
					display = ".";
				} else if(v == FIELD_WALL) {
					display = "#";
				} else {
					display = (v - 1).toString();
				}
				chunk.push(display.padStart(2, " "));
			}
...
/** @type {HeuristicCalculator} */
const heuristicDepth = state => state.getDepth();
/** @type {HeuristicCalculator} */
const heuristicConstant = _ => 0;
/** @type {HeuristicCalculator} */
const heuristicCarBlocked = state => {
	const width = state.getWidth();
...
const heuristicUCS = heuristicDepth;
const heuristicGBFSCarBlocked = heuristicCarBlocked;
const heuristicAStarCarBlocked = s => heuristicDepth(s) + heuristicCarBlocked(s);

const stringSplitAll = s => {
	const result = new Array(s.length).fill();
	for(let i = 0; i < s.length; i++)
		result[i] = s.charAt(i);
	return result;
};
function parseBoardInput(inputText) {
	const lines = inputText.trim().split("\n").map(l => l.trim());
	const [width, height] = lines[0].split(" ").map(s => parseInt(s));
	const carCount = parseInt(lines[1], 10);
	const boardLines = lines.slice(2).map(l => stringSplitAll(l));
	if(isNaN(width))
	...
	for(let y = 0; y < boardLines.length; y++) {
		for(let x = 0; x < boardLines[y].length; x++) {
			...
			if(ch == "K") {
				...
				const exitX = Math.max(0, Math.min(width - 1, x));
				const exitY = Math.max(0, Math.min(height - 1, y));
				if(exitX != width - 1 && exitY != height - 1)
					throw new Error("Exit position is not on the edge");
				exitPosition = exitY * width + exitX;
				continue;
			}
			if(ch == "#") {
				walls.push([x, y]);
	...
	const cars = [];
	for(const [ch, cells] of carCells) {
		...
		for(let i = 0; i < cells.length; i++) {
			const [x, y] = cells[i];
			if(y != y0) horizontal = false;
			if(x != x0) vertical = false;
		}
		const size = cells.length;
		if(!horizontal && !vertical)
			throw new Error(`Car piece ${ch} has inconsistent shape`);
		cars.push({ symbol: ch, direction: horizontal ? HORIZONTAL : VERTICAL, positionX: x0, positionY: y0, size: size });
	}
	...
	// Manually build carsWithIds
	const carsWithIds = [];
	for (let i = 0; i < cars.length; i++) {
		const c = cars[i];
		carsWithIds.push({ symbol: c.symbol, id: i, direction: c.direction, size: c.size });
	}
	// Manually build carPositions
	const carPositions = [];
	for (let i = 0; i < cars.length; i++) {
		carPositions.push(cars[i].positionY * width + cars[i].positionX);
	}
	// Manually build wall positions
	const wallPositions = [];
	for (let i = 0; i < walls.length; i++) {
		const [x, y] = walls[i];
		wallPositions.push(y * width + x);
	}
	return { width: width, height: height, cars: carsWithIds, carPositions: carPositions, walls: wallPositions, exitPosition: exitPosition };
}

export function Porffor_runSolver() {
	let arg = "";
	Porffor.readArgv(1, arg);
	const [heuristicStr, boardStr] = arg.split("\0");
	const heuristic = parseInt(heuristicStr);
	const board = parseBoardInput(boardStr);
	const heuristicCalculator = [ heuristicUCS, heuristicGBFSCarBlocked, heuristicAStarCarBlocked ][heuristic];
	const state = State.new_root( board.width,  board.height,  board.cars,  board.carPositions,   board.walls,  board.exitPosition );
	const solver = new Solver(heuristicCalculator, state);
	...
	return ({ duration: end - start, board: board, visitedNodes: solver.getVisitedNodes(), solutionSteps: solver.getSolution() == null ? null : solver.getSolution().getStepDescription() });
}
...
// 	height: 6,
// 	cars: [
// 		{ symbol: 'P', id: 0, direction: 0, size: 2 },
// 		{ symbol: 'B', id: 1, direction: 1, size: 2 },
// 		{ symbol: 'F', id: 2, direction: 1, size: 3 },
// 		{ symbol: 'C', id: 3, direction: 1, size: 2 },
// 		{ symbol: 'D', id: 4, direction: 1, size: 2 },
// 		{ symbol: 'G', id: 5, direction: 1, size: 3 },
// 		{ symbol: 'A', id: 6, direction: 0, size: 2 },
// 		{ symbol: 'H', id: 7, direction: 1, size: 2 },
// 		{ symbol: 'I', id: 8, direction: 0, size: 3 },
// 		{ symbol: 'J', id: 9, direction: 1, size: 2 },
// 		{ symbol: 'L', id: 10, direction: 0, size: 2 },
// 		{ symbol: 'M', id: 11, direction: 0, size: 2 }
// 	],
// 	carPositions: [
// 		13,  2,  5,  9, 10,
// 		12,  0, 19, 21, 26,
...
// 	board.exitPosition
// );
// const solver = new Solver(heuristicAStarCarBlocked, state);
// const start = performance.now();
// let currentTick = 0;
// while(solver.tick()) {
...
// 	// @Porffor
// 	solutionSteps: solver.getSolution() == null ? null : solver.getSolution().getStepDescription()
// });
